...
import { deskPublishDeed } from "@/lib/desk/deed-definitions";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const deed = await deskPublishDeed(deedId, identity);
    return deskJson({ ok: true, deed });
  } catch (error) {
    return mapDeskError(error, "POST /api/desk/deeds/[deedId]/publish");
  }
}
