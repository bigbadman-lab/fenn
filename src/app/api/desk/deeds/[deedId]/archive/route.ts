...
import { deskArchiveDeed } from "@/lib/desk/deed-definitions";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const deed = await deskArchiveDeed(deedId, identity);
    return deskJson({ ok: true, deed });
  } catch (error) {
    return mapDeskError(error, "POST /api/desk/deeds/[deedId]/archive");
  }
}
