...
import { deskResolveCampaign, getDeskCampaign } from "@/lib/desk/hollow";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    await deskResolveCampaign(id, identity.actorId);
    const campaign = await getDeskCampaign(id);
    return deskJson({ ok: true, campaign });
  } catch (error) {
    return mapDeskError(error, "POST /api/desk/hollow/campaigns/[id]/resolve");
  }
}
