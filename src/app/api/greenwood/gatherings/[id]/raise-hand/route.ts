...
import { raiseGatheringHand } from "@/lib/greenwood/gatherings/member-ops";
...
/**
 * POST /api/greenwood/gatherings/[id]/raise-hand
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const gathering = await raiseGatheringHand(id, profile.id, admin);
    return NextResponse.json(
      { ok: true, gathering },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (error) {
    return mapError(error, "POST raise-hand");
  }
}
