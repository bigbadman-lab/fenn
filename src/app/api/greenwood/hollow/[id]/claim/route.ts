...
import { claimHollowLeaf } from "@/lib/greenwood/hollow/member-ops";
...
/**
 * POST /api/greenwood/hollow/[id]/claim
 * Empty body. LEAF only. Profile from Privy.
 */
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await claimHollowLeaf(id, profile.id, admin);
    return NextResponse.json(
      {
        ok: true,
        reward: result.reward,
        leafBalance: result.leafBalance,
        leafLifetimeEarned: result.leafLifetimeEarned,
    ...
    console.error("[POST hollow claim]", error);
    return NextResponse.json(
