import {
  isNormalizedEvmAddress,
  normalizeEvmAddress,
} from "@/lib/wallet/evm";

/**
 * Parse a comma-separated EVM wallet allowlist.
 *
 * - Trims whitespace, ignores empty entries
 * - Normalizes to lowercase 0x…
 * - Deduplicates
 * - Empty / null / undefined → []
 * - Any malformed non-empty entry throws (fail-loud)
 *
 * Membership must use {@link isWalletInEvmAllowlist} (exact match after normalize).
 * Never log or return allowlist contents to clients.
 */
export function parseEvmWalletAllowlist(
  raw: string | null | undefined,
  label: string,
): string[] {
  if (raw == null) return [];

  const entries = raw.split(",");
  const allowlist: string[] = [];
  const seen = new Set<string>();

  for (const entry of entries) {
    const trimmed = entry.trim();
    if (trimmed.length === 0) continue;

    const normalized = normalizeEvmAddress(trimmed);
    if (!isNormalizedEvmAddress(normalized)) {
      throw new Error(`Invalid address in ${label}: "${trimmed}"`);
    }
    if (seen.has(normalized)) continue;
    seen.add(normalized);
    allowlist.push(normalized);
  }

  return allowlist;
}

/** Exact membership after lowercase normalisation. No substring matching. */
export function isWalletInEvmAllowlist(
  walletAddress: string,
  allowlist: readonly string[],
): boolean {
  const normalized = normalizeEvmAddress(walletAddress);
  if (!isNormalizedEvmAddress(normalized)) return false;
  return allowlist.includes(normalized);
}
