-- VELL identity — Solana wallet addresses for login / profiles
-- LOCAL ONLY until manually applied in Supabase SQL Editor.
-- Requires a clean identity layer: existing EVM profile rows must be migrated or wiped
-- before applying CHECK constraint changes.

CREATE OR REPLACE FUNCTION public.is_normalized_solana_address(value text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT value ~ '^[1-9A-HJ-NP-Za-km-z]{32,44}$';
$$;

COMMENT ON FUNCTION public.is_normalized_solana_address(text) IS
  'True when value matches a base58 Solana wallet address (32–44 chars).';

ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_wallet_address_normalized;
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_wallet_address_normalized
  CHECK (public.is_normalized_solana_address(wallet_address));

ALTER TABLE public.leaf_ledger
  DROP CONSTRAINT IF EXISTS leaf_ledger_wallet_address_normalized;
ALTER TABLE public.leaf_ledger
  ADD CONSTRAINT leaf_ledger_wallet_address_normalized
  CHECK (public.is_normalized_solana_address(wallet_address));

ALTER TABLE public.circulation_recipients
  DROP CONSTRAINT IF EXISTS circulation_recipients_wallet_normalized;
ALTER TABLE public.circulation_recipients
  ADD CONSTRAINT circulation_recipients_wallet_normalized
  CHECK (public.is_normalized_solana_address(wallet_address));

ALTER TABLE public.greenwood_reward_campaign_recipients
  DROP CONSTRAINT IF EXISTS greenwood_reward_campaign_recipients_wallet_normalized;
ALTER TABLE public.greenwood_reward_campaign_recipients
  ADD CONSTRAINT greenwood_reward_campaign_recipients_wallet_normalized
  CHECK (
    wallet_address_snapshot IS NULL
    OR public.is_normalized_solana_address(wallet_address_snapshot)
  );

ALTER TABLE public.greenwood_hollow_rewards
  DROP CONSTRAINT IF EXISTS greenwood_hollow_rewards_wallet_normalized;
ALTER TABLE public.greenwood_hollow_rewards
  ADD CONSTRAINT greenwood_hollow_rewards_wallet_normalized
  CHECK (
    wallet_address_snapshot IS NULL
    OR public.is_normalized_solana_address(wallet_address_snapshot)
  );
...
BEGIN
  ...
  v_wallet := trim(p_wallet_address);
  IF NOT public.is_normalized_solana_address(v_wallet) THEN
